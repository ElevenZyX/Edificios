...
function Delivery() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://proyectobuildingbuddy.azurewebsites.net/api/deliveries",
        delivery,
        {
          headers: {
  ...
  const sendSMS = async (phone) => {
    try {
      await axios.post("https://proyectobuildingbuddy.azurewebsites.net/api/send-sms", {
        phone,
    ...
    const fetchDepartments = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `https://proyectobuildingbuddy.azurewebsites.net/api/departments/${user._id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    const fetchDepartmentPhone = async () => {
      if (selectedDepartment) {
        try {
          const token = localStorage.getItem("token");
          const response = await axios.get(
            `https://proyectobuildingbuddy.azurewebsites.net/api/department/${selectedDepartment}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
